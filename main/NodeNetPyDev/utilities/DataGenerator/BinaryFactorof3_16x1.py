...
for x in range(0, datasize):
    randint = random.randint(0, 2**16-1)
...
datamtrxstring = str(datasize)+' 16\n'
validationdatamtrxstring = str(validationdatasize)+' 16\n'
for x in range(0, datasize):
    string = ''
    for y in range(0, 16):
        string = string +' '+ str(bin(datalist[x]).lstrip('-0b').zfill(16))[y]
    datamtrxstring = datamtrxstring + string +'\n'

for x in range(0, validationdatasize):
    string = ''
    for y in range(0, 16):
        string = string +' '+ str(bin(validationdatalist[x]).lstrip('-0b').zfill(16))[y]
