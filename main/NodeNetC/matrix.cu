...
__global__ static void matrix_CUDA_multi(int *m1_row, int *m1_column, double *m1_elem, int *m2_row, int *m2_column, double *m2_elem, double *result) {
  ...
  int i, j;
  for(i = tid; i < *m1_row * *m2_column; i += THREAD_NUM) {
    double sum = 0;
    int row = i / *m2_column;
    int column = i % *m2_column;
    for(j = 0; j < *m1_column; j++) {
      sum += m1_elem[row * *m1_column + j] * m2_elem[column + j * *m2_column];
    }
    result[i] = sum;
  }
  __syncthreads();
...
matrix operator *(const matrix& m1, const matrix& m2) {
  ...
  if(cuda_enabled == 1) {
    int *CUDA_m1_row, *CUDA_m1_column, *CUDA_m2_row, *CUDA_m2_column;
    double *CUDA_m1_elem, *CUDA_m2_elem, *CUDA_result;
    cudaMalloc((void**) &CUDA_m1_elem, sizeof(double) * m1.row * m1.column);
    cudaMalloc((void**) &CUDA_m2_elem, sizeof(double) * m2.row * m2.column);
    cudaMalloc((void**) &CUDA_m1_row, sizeof(int));
    cudaMalloc((void**) &CUDA_m1_column, sizeof(int));
    cudaMalloc((void**) &CUDA_m2_row, sizeof(int));
    cudaMalloc((void**) &CUDA_m2_column, sizeof(int));
    cudaMalloc((void**) &CUDA_result, sizeof(double) * m1.row * m2.column);

    cudaMemcpy(CUDA_m1_elem, m1.a, sizeof(double) * m1.row * m1.column, cudaMemcpyHostToDevice);
    cudaMemcpy(CUDA_m2_elem, m2.a, sizeof(double) * m2.row * m2.column, cudaMemcpyHostToDevice);
    cudaMemcpy(CUDA_m1_row, &m1.row, sizeof(int) , cudaMemcpyHostToDevice);
    cudaMemcpy(CUDA_m1_column, &m1.column, sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(CUDA_m2_row, &m2.row, sizeof(int), cudaMemcpyHostToDevice);
    cudaMemcpy(CUDA_m2_column, &m2.column, sizeof(int), cudaMemcpyHostToDevice);

    matrix_CUDA_multi<<<BLOCK_NUM, THREAD_NUM, THREAD_NUM * sizeof(int)>>>(CUDA_m1_row, CUDA_m1_column, CUDA_m1_elem, CUDA_m2_row, CUDA_m2_column, CUDA_m2_elem, CUDA_result);

    double result[m1.row * m2.column];
    cudaMemcpy(result, CUDA_result, sizeof(double) * m1.row * m2.column, cudaMemcpyDeviceToHost);
    answer.setmatrix(m1.row, m2.column, result);

    cudaFree(CUDA_m1_elem);
    cudaFree(CUDA_m2_elem);
    cudaFree(CUDA_m1_row);
    cudaFree(CUDA_m1_column);
    cudaFree(CUDA_m2_row);
    cudaFree(CUDA_m2_column);
