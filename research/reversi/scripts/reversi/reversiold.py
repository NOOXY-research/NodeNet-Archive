...
class ReversiMappingContainer(object):

    def __init__(self):
        self.ReversiMappings = []
        self.Contains = []
    def __str__(self):
        string = ''
        for x in range(len(self.ReversiMappings)):
            string += str(self.ReversiMappings[x])+' '+str(self.Contains[x])+'\n'
        return string
    def index(self, mapping):
        return self.index_handler(mapping, 0, len(self.ReversiMappings)-1)
    def index_handler(self, mapping, start, end):
        if len(self.ReversiMappings)==0:
            return None
        middle = int((start+end)/2)
        order = self.bigger(mapping, self.ReversiMappings[middle])
        if order == 0:
            return middle
        elif order == -1:
            if start-end == 0:
                return None
            return self.index_handler(mapping, start, middle)
        else:
            if start-end == 0:
                return None
            return self.index_handler(mapping, middle+1, end)


    def insertindex(self, mapping):
        return self.insertindex_handler(mapping, 0, len(self.ReversiMappings)-1)

    def insertindex_handler(self, mapping, start, end):
        if(len(self.ReversiMappings)==0):
            return 0
        middle = int((start+end)/2)
        order = self.bigger(mapping, self.ReversiMappings[middle])
        ...
        if order == 0:
            ...
            if start-end == 0:
                return middle
            return self.insertindex_handler(mapping, start, middle)
        else:
            if start-end == 0:
                return middle
            return self.insertindex_handler(mapping, middle+1, end)

    def bigger(self, mapping1, mapping2):
        for x in range(len(mapping1)):
            if mapping1[x]>mapping2[x]:
                return 1
            elif mapping1[x]<mapping2[x]:
                return -1
        return 0

    def setcontain(self, mapping, contain):
        if self.index(mapping) == None:
            index = self.insertindex(mapping)
            self.ReversiMappings.insert(index, mapping)
            self.Contains.insert(index, [])
        self.Contains[self.index(mapping)] = contain
    def getcontain(self, mapping):
        return self.Contains[self.index(mapping)]
    def getcontainslist(self):
        return self.Contains
    def getreversimappings(self):
        return self.ReversiMappings
    ...
    def dumptomtrx(self):
        ...
        for x in range(len(finalinputmapping)):
            ...
            if finalinputmapping[x] in self.LoseMappings:
                ...
                base = location
            if base == 1 and location == 1:
                (finaloutputvalues[x])[0] = 5*0.82
            elif base == 1 and location == 0:
                (finaloutputvalues[x])[0] = 5*(1-0.82)
            else:
                (finaloutputvalues[x])[0] = 5*(location / base)
            # print(location)
            # input()
...
class ReversiPolicyDropsRecord(object):
    ...
    def dumptomtrx(self):
        outputdata = ReversiMappingContainer()
        finalinputmapping = []
